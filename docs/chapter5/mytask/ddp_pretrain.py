...
本脚本用于语言模型的预训练阶段，支持多GPU并行计算

主要功能：
1. 使用PretrainDataset加载预训练数据
2. 使用混合精度训练（bfloat16/float16）加速训练
3. 支持梯度累积，模拟更大的batch size
4. 使用余弦退火学习率调度策略
5. 支持多GPU并行训练（DataParallel）
6. 集成SwanLab进行实验跟踪和可视化

训练流程：
1. 初始化模型和分词器
2. 创建数据加载器
3. 设置优化器和学习率调度
4. 执行训练循环（前向传播、反向传播、参数更新）
5. 定期保存模型检查点
"""
import os
import argparse
...
from docs.chapter5.mytask.dataset import PretrainDataset
from docs.chapter5.mytask.k_model import ModelConfig, Transformer
...
if __name__ == "__main__":
    ...
    ArgumentParser 类负责定义、解析和验证命令行参数
    description="Tiny-LLM Pretraining"：设置程序的描述信息
    当用户运行 python ddp_pretrain.py --help 时，会显示这段描述
    帮助用户了解这个脚本的用途
    """
