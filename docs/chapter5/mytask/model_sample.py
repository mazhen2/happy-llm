...
本模块提供了TextGenerator类，用于加载训练好的模型并进行文本生成。

主要功能：
1. 加载预训练或SFT模型
2. 支持预训练模型的文本续写
3. 支持SFT模型的对话生成
4. 支持多种采样策略（temperature, top_k）
5. 支持批量生成多个样本
...
class TextGenerator:
    ...
    def chat_template(self, prompt):
        ...
        使用tokenizer的chat_template将用户输入转换为模型输入格式。
        会自动添加system消息和user消息，并添加生成提示。

        Args:
            prompt (str): 用户输入的提示文本

        Returns:
            str: 格式化后的对话文本，可以直接用于模型输入
        """
        # 构造对话消息列表
        message = [
        ...
        # 使用chat_template转换为模型输入格式
        # tokenize=False: 返回文本而不是token ID
        # add_generation_prompt=True: 在末尾添加assistant生成提示
...
if __name__ == "__main__":
    ...
    # 预训练模型的提示数据（直接使用文本，不需要chat_template）
    ...
    for i in range(len(pretrain_prompt_datas)):
        samples = generator.pretrain_sample(
            start=pretrain_prompt_datas[i],
            num_samples=1,  # 生成1个样本
            max_new_tokens=120,  # 最多生成120个新token
            temperature=0.75  # 温度参数，控制随机性
        )
        # 打印生成的样本
    ...
    for i in range(len(sft_prompt_datas)):
        samples = generator.sft_sample(
            start=sft_prompt_datas[i],
            num_samples=1,  # 生成1个样本
            max_new_tokens=128,  # 最多生成128个新token
            temperature=0.6  # 温度参数，控制随机性（SFT通常使用较低温度）
        )
        # 打印问题和生成的回复
